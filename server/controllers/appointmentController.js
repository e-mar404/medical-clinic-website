...
async function getClinicAppointments(res, db, req) {
  try {
    ...
    db.query('SELECT doctor_id, patient_id, appointment_date, appointment_time, appointment_status, appointment_id FROM Appointment WHERE clinic_id = ?', [clinicId], async (err, results) => {
      if (err) {
        console.error(err);
